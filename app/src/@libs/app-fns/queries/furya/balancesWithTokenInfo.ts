...
import { furyaBalancesQuery } from './balances';

export type FuryaBalancesWithTokenInfo = {
  tokens: Array<{
...
export async function furyaBalancesWithTokenInfoQuery(
  walletAddr: HumanAddr | undefined,
  assets: terraswap.AssetInfo[],
  queryClient: QueryClient,
): Promise<FuryaBalancesWithTokenInfo> {
  const { balances } = await furyaBalancesQuery(
    walletAddr,
    assets,
    queryClient,
  );
