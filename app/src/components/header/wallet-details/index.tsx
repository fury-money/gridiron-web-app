import { useFuryaNativeBalances } from '@libs/app-provider';
...
function WalletDetailsBase({
  ...
  const { uUST, uLuna } = useFuryaNativeBalances(
    connectedWallet?.walletAddress,
  );

  const uNEB = useNebBalance(connectedWallet?.walletAddress);

  const viewOnFuryaFinder = useCallback(() => {
    window.open(
  ...
  return (
    <div className={className}>
      <header>
        <Tooltip title="View on Furya Finder" placement="top">
          <h2 onClick={viewOnFuryaFinder}>
